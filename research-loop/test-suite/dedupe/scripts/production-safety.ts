/**
 * dedupe/scripts/production-safety.ts
 *
 * Runs the production-safety grouping tests for the deduplication engine.
 * These tests verify invariants that must hold in production:
 *   - deterministic hash output
 *   - card/card with different accounts do not merge
 *   - account/account with different accounts do not merge
 *   - missing-account non-bridge does not overmerge
 *   - BNPL/wallet bridge still merges with bank debit
 *   - processing order does not affect grouping results
 *
 * To run:
 *   npx tsx research-loop/test-suite/dedupe/scripts/production-safety.ts
 */

import { dedupeTransactions, generateSMSHash, TransactionInput } from "../../../../src/features/sms-parser/engine";
import { ParsedTransaction } from "../../../../src/features/sms-parser/types";
...
function runTests() {
  ...
  const inputsCard: TransactionInput[] = [
  ...
  const inputsAccount: TransactionInput[] = [
  ...
  const inputsMissing: TransactionInput[] = [
  ...
  const inputsBridge: TransactionInput[] = [
